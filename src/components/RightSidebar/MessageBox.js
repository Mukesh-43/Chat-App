...
function MessageBox(props) {
  const [chat, setChat] = useState([]);      // we're using useState hook
  const [length, setLength] = useState();    // we're using useState hook

  const dispatch = useDispatch();            // dispatch function works according to action type
            ...
            {chat.map((m, index) =>
              m.sender === "me" ? (
                <RightChatBubble
                  ...
                  image={"https://w7.pngwing.com/pngs/639/327/png-transparent-naruto-uzumaki-sasuke-uchiha-kakashi-hatake-asuma-sarutobi-naruto-shippuden-naruto-vs-sasuke-naruto-cartoon-fictional-character-naruto-thumbnail.png"}
                />
              ) : (
                <LeftChatBubble
                  message={m}
